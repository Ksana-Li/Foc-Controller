...
FocController::FocController() {
	...
	adcValue = new AdcValue(&hadc1, &hadc2, &hadc3);
	this->Adc = adcValue;

	sensor = new MagneticSensorSPI(&hspi1, GPIOA, GPIO_PIN_4, 0x3fff, 16384, MagneticSensorSPI::CW);
	//sensor = new IncrementalEncoder(&htim3, 64000, MagneticSensorSPI::CCW);
	this->Sensor = sensor;

	svpwm = new SvPwm(&htim1, 5600);
	this->Svpwm = svpwm;

	focConv = new FocConverter();
	this->FocConv = focConv;
